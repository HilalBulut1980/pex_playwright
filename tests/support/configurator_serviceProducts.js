...
exports.ServiceProductConfigurator = class ServiceProductConfigurator {
    ...
    async configureProduct(testcase) {
        ...
        if (testcase.service == "Längere Führungsschnüre") {

            await this.page.locator('.additional_option_item:nth-child(1) input').fill(testcase.produktname);
            await this.page.locator('.additional_option_item:nth-child(2) input').fill(testcase.schienenfarbe);
            await this.page.locator('.additional_option_item:nth-child(3) input').fill(testcase.breite);
            await this.page.locator('.additional_option_item:nth-child(4) input').fill(testcase.hoehe);
            await this.page.locator('.additional_option_item:nth-child(5) input').fill(testcase.laenge_neu);
            await this.page.selectOption(('.additional_option_item:nth-child(6) select'), { label: testcase.seite });
            await this.page.locator('.additional_option_item:nth-child(7) textarea').fill(testcase.anmerkungen);
        }
        else if (testcase.service == "Schnur Ersetzen" || testcase.service == "Änderungsauftrag Schnurlänge") {

            await this.page.locator('.additional_option_item:nth-child(1) input').fill(testcase.bestellnummer);
            await this.page.locator('.additional_option_item:nth-child(2) input').fill(testcase.produkt);
            await this.page.locator('.additional_option_item:nth-child(3) input').fill(testcase.breite);
            await this.page.locator('.additional_option_item:nth-child(4) input').fill(testcase.hoehe);
            await this.page.locator('.additional_option_item:nth-child(5) input').fill(testcase.laenge_neu);
        }
        else if (testcase.service == "Änderungsauftrag Breite Kürzen") {

            await this.page.locator('.additional_option_item:nth-child(1) input').fill(testcase.bestellnummer);
            await this.page.locator('.additional_option_item:nth-child(2) input').fill(testcase.produkt);
            await this.page.locator('.additional_option_item:nth-child(3) input').fill(testcase.breite);
            await this.page.locator('.additional_option_item:nth-child(4) input').fill(testcase.hoehe);
            await this.page.locator('.additional_option_item:nth-child(5) input').fill(testcase.kuerzung);
        }
